...
function handleConnection(ws, req) {
  ...
  const geminiCallbacks = {
    ...
    onFunctionCall: async ({ id, name, args }) => {
      ...
      const result = await handleTicketCreation(args);
  ...
  async function handleTicketCreation(args) {
    ...
    const ticket = await createTicket(name, phone, complaint, severity_score, 'public');
