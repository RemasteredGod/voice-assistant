...
async function addFile(ticketId, fileMetadata) {
  if (hasPostgres) {
    await query(
      `INSERT INTO ticket_files (id, ticket_id, filename, original_name, mime_type, uploaded_at)
       VALUES ($1,$2,$3,$4,$5,$6)`,
      [crypto.randomUUID(), ticketId, fileMetadata.filename, fileMetadata.originalName, fileMetadata.mimeType, fileMetadata.uploadedAt || new Date().toISOString()],
    );
    return;
  }
